...
parameters <- read.csv("cluster/data/params.csv", header=TRUE)
...
for (i in which(parameters$plot, isTRUE(parameters$plot))){
  optim <- readRDS(paste0("cluster/results/optimisations/param_tests/output", i))
...
pdf("cluster/results/optimisations/scores_compiled", width=16, height=20)
plot(GNB)
plot(TNB)
plot(GNS)
dev.off()
...
for (i in which(parameters$plot, isTRUE(parameters$plot))){
  optim <- readRDS(paste0("cluster/results/optimisations/output", i))
  scores <- (optim[["optim"]][["scores"]])
...
pdf("cluster/results/optimisations/max_fitness_plots", width=12, height=9)
plot(GNBMax)
plot(TNBMax)
plot(GNSMax)
dev.off()
...
for (i in 1:10){
  optim <- readRDS(paste0("cluster/results/optimisations/param_tests/cprob", i))
  scores <- (optim[["optim"]][["scores"]])
  scores[,2:4] <- -scores[,2:4]
