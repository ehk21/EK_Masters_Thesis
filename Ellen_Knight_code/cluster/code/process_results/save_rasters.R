...
source("cluster/code/process/output_optim_rasters.R")

optimsDF <- read.csv("cluster/data/optims.csv")
...
# initiate pdf...
pdf("cluster/results/optimisations/TNB_rasters", width=20, height=55)

for(j in c(201:226)){
  if(isTRUE(optimsDF$complete[j])){
    
    # load results
    result <- readRDS(paste0("cluster/results/optimisations/results/", optimsDF$optim[j], optimsDF$number[j]))
