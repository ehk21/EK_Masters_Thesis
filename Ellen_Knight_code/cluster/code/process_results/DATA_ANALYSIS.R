...
source("cluster/code/process/load_optims_fn.R")
source("cluster/code/process/pixel_plots.R")
source("cluster/code/process/scores_plots.R")
source("cluster/code/process/plot_floral_nesting_scores.R")
source("cluster/code/process/landscapemetrics_plots.R")
...
results <- readRDS("cluster/results/results")
...
ggsave("../write_up/Figures/scale_interaction_plot.png", dpi=600, width=21, height=15, units="cm")
...
write.csv(modeldf, "cluster/results/GNB_radius_comparison.csv")

ANOVAdf <- tidy(anova(model))
write.csv(ANOVAdf, "cluster/results/GNB_radius_comparison_ANOVA.csv")
...
scaleNF <- plot.floral.nesting.scores (scores=nfScores, byoptim=c("GNB"), byradius=c(500 ,1000, 2000), 
                                     byguild=c("GNB"), byscore = c("Spring", "Summer", "Nesting"), xby="radius",
                                     compareby="radius", facetby="score", horizontal=T, plotTitle="C", bypopulation=c("compare"), legendTitle="Population", xlab="Radius")
...
write.csv(ANOVAdf, "cluster/results/GNB_NF_scores_comparison_Tukey.csv")
...
singleObjPix1 <- singleObjPix + theme(legend.position="bottom", 
                                      ...
                                      plot.title=element_text(size=16, face="bold"))
plot(singleObjPix1)
ggsave("../write_up/Figures/singleObjPix.png", dpi=600, width=24, height=16, units="cm")
...
multiObjPix1 <- multiObjPix + theme(legend.position="bottom", 
                                   ...
                                   plot.title=element_text(size=16, face="bold"))
plot(multiObjPix1)
ggsave("../write_up/Figures/multiObjPix.png", dpi=600, width=24, height=16, units="cm")
...
#plot
MultSingCompPlot <- ggplot(data2, aes(y=perc, x=Landclass_ExpOp_GFS, fill=type)) +
...
ggsave("../write_up/Figures/multiSingCompPlot.png", dpi=600, width=21, height=27, units="cm")
...
ggsave("../write_up/Figures/PCPBee.png", dpi=600, width=14, height=8, units="cm")
...
plot(farmerPCP)
ggsave("../write_up/Figures/PCPFarmer.png", dpi=600, width=14, height=8, units="cm")
...
results <- readRDS("cluster/results/results2")
patchDensity <- results$patchDensity
...
colors <- c("#B66DFF", "#FFFF6D", "#000000", "#004949", "#924900", "#24FF24", "#6DB6FF", "#FFB6DB")

plot <- ggplot(data, aes(x=Landclass_ExpOp_GFS, y=Score, fill=Landclass_ExpOp_GFS)) +
...
plot(plot)
ggsave("../write_up/Figures/NF_scores.png", dpi=600, width=21, height=15, units="cm")
